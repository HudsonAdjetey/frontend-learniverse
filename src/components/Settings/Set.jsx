...
import "../../container/styles/settings.css";
import {
  getDownloadURL,
  getStorage,
  ref,
...
const Set = () => {
  ...
  const handleSetImage = async (e) => {
    ...
    if (sizeInMb > 3) {
      toastError("Choose less than 3MB sized image");
      setImageUpload(null);
      return;
    }
  ...
  const mutation = useMutation({
    mutationFn: async (body) => {
      const res = await axios.put(`${URL}/banner-upload`, body);
      return res;
    },
  });
  ...
  return (
    <div className="content__whole profile__page settings">
      ...
      <div className="compactContent">
        ...
        <div className="inner__co flex-row">
          <div className="account__input">
            <label htmlFor="usernameComp">Username</label>
            <input type="text" id="usernameComp" />
          </div>

          <div className="account__input">
            <label htmlFor="roleComp">Role</label>
            <input type="text" id="roleComp" value={"Admin"} readOnly />
          </div>
        </div>
        <div className="inner__co flex-row">
          <div className="account__input">
            <label htmlFor="previousPas">Previous Password</label>
            <input type="password" id="previousPas" />
          </div>

          <div className="account__input">
            <label htmlFor="currentPas">Current Password</label>
            <input type="password" id="currentPas" />
          </div>
        </div>
      </div>
      {/* USER'S ACCOUNT */}
